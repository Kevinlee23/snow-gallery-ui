...
class MapboxMapInstance implements MapInstance {
  ...
  addMarker(marker: MapMarker): void {
    ...
    // 创建弹窗
    const popup = new mapboxgl.Popup({ offset: 25 }).setHTML(marker.content)

    // 创建标记
    const mapboxMarker = new mapboxgl.Marker(el).setLngLat([marker.position.lng, marker.position.lat]).setPopup(popup).addTo(this.map)
  ...
  updateMarkersWithDistance(currentLocation: MapPosition): void {
    this.markers.forEach((marker) => {
      ...
      // 计算距离（使用简单的球面距离公式）
      const distance = this.calculateDistance(currentLocation.lat, currentLocation.lng, markerLngLat.lat, markerLngLat.lng)

      const popup = marker.getPopup()

      if (popup) {
        const originalContent = popup._content?.innerHTML
...
export function useMapboxMap(options: MapHookOptions): MapHookReturn {
  ...
  const onLocationFound = (position: GeolocationPosition) => {
    ...
    const hasSignificantChange =
      !currentLocation.value || Math.abs(currentLocation.value.lat - lat) > 0.0005 || Math.abs(currentLocation.value.lng - lng) > 0.0005
