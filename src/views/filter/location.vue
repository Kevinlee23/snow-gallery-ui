...
const { filterList, filterValue, getFilterList } = useFilterLocal('LOCATION')

// 数据请求
const photos = ref<any[]>([])
const total = ref<number>(1)
onMounted(async () => {
  const res: Response<{ list: Photo[]; total: number }> = await request.post('/gallery/photo/list', { location: filterValue })
